...
async def query_aerich_all_by_limit(
    query: schema.Aerich = Depends(), page_number: int = 1, page_size: int = 10
) -> schema.PageResult[schema.Aerich]:
    total = await dao.count(**query.model_dump(exclude_none=True))
    data = await dao.query_all_by_limit(
...
async def create_aerich(instance: schema.Aerich) -> schema.Result[schema.Aerich]:
    return schema.Result.ok(await dao.create(instance))


@aerich.patch("/{id}", summary="更新数据")
async def update_aerich_by_id(
    id: int, instance: schema.Aerich
) -> schema.Result[schema.Aerich]:
